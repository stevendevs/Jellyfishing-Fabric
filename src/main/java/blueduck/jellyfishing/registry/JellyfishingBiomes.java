...
public class JellyfishingBiomes {
    ...
    public static void init() {
        CLOUDS = JellyfishingConfig.get().biome.flower_clouds ? 0.000005F : 0f;
        MUSIC = JellyfishingConfig.get().client.music ? new MusicSound(JellyfishingSounds.BACKGROUND_MUSIC, 200, 4000, false) : MusicType.GAME;

        JELLYFISH_FIELDS = createBiome("jellyfish_fields", makeJellyfishingBiome());

        OverworldBiomes.addContinentalBiome(JELLYFISH_FIELDS, OverworldClimate.TEMPERATE, JellyfishingConfig.get().jellyfish_fields_weight);
    }
